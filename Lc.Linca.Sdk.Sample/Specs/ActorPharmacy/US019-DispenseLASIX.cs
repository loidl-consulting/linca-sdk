...
internal class US019_DispenseLasix : Spec
{
    ...
    private bool CreateMedicationDispenseRecord()
    {
        ...
        (Bundle orders, bool received) = LincaDataExchange.GetPrescriptionToDispense(Connection, "1111BBBB3333");
        ...
        if (received)
        {
            ...
            {
                Reference = $"LINCAPrescriptionMedicationRequest/{prescriptionLasix.Id}"
            });

            dispense1.Status = MedicationDispense.MedicationDispenseStatusCodes.Completed;
            dispense1.Subject = prescriptionLasix.Subject;
            ...
            dispense1.DosageInstruction = prescriptionLasix?.DosageInstruction;

            dispense1.Note = new()
            {
                new() { Text = "Achtung: höherer Wirkstoffgehalt, Dosierung anpassen: 1/2-0-0-0" }
            };
