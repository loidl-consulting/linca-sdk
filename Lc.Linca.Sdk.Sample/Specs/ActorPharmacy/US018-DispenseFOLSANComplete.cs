...
internal class US018_DispenseFolsanComplete : Spec
{
    ...
    public US018_DispenseFolsanComplete(LincaConnection conn) : base(conn)
    {
        Steps = new Step[]
        {
    ...
    private bool CreateMedicationDispenseRecord()
    {
        ...
        (Bundle orders, bool received) = LincaDataExchange.GetPrescriptionToDispense(Connection, "AAAABBBBCCCC");

        if (received)
        {
            List<MedicationRequest> prescriptionsToDispense = BundleHelper.FilterPrescriptionsToDispense(orders);

            MedicationRequest? prescriptionFolsan = prescriptionsToDispense.Find(x => x.Medication.Concept.Coding.First().Code.Equals("0713987"));

            if (prescriptionFolsan == null)
            {
            ...
            {
                Reference = $"LINCAPrescriptionMedicationRequest/{prescriptionFolsan.Id}"
            });

            dispense1.Status = MedicationDispense.MedicationDispenseStatusCodes.Completed;
            dispense1.Subject = prescriptionFolsan.Subject;
            dispense1.Medication = prescriptionFolsan.Medication;
            /*
            ...
            */
            ...
            dispense1.DosageInstruction = prescriptionFolsan?.DosageInstruction;
            ...
            dispense1.Type = new()
            {
                Coding = new()
                {
                    new Coding(system: "http://terminology.hl7.org/CodeSystem/v3-ActCode", code: "RFC") // complete the dispense
                }
            };
